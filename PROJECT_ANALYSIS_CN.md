...
<!--
文档说明：
本文档详细介绍了 Shannon 渗透测试框架的工作原理和技术架构。
主要内容包括：
1. 项目概述和核心特点
2. 技术架构（四层架构设计）
3. 五阶段工作流程详解（预侦察、侦察、漏洞分析、漏洞利用、报告生成）
4. Temporal 工作流编排机制
5. Claude Agent SDK 集成细节
6. Git 检查点管理、审计系统、配置系统等关键组件
7. 性能优化策略和基准测试结果

阅读建议：
- 开发人员：重点阅读"技术架构"和"Temporal 工作流编排"部分
- 安全研究人员：重点阅读"工作流程详解"和"提示模板系统"部分
- 架构师：重点阅读"技术架构"和"性能优化"部分
-->
...
<!--
技术要点：
Temporal 是 Shannon 的核心编排引擎，提供以下关键能力：
1. 持久化工作流状态 - 工作流状态存储在数据库中，不会因进程重启而丢失
2. 自动重试机制 - 区分可重试错误和不可重试错误，使用指数退避算法
3. 崩溃恢复 - Worker 进程崩溃后，新的 Worker 可以从上次断点继续执行
4. 实时查询 - 通过 Query 机制实时查询工作流进度和状态
5. 事件溯源 - 完整记录所有工作流事件，支持回放和调试

与传统队列系统（如 RabbitMQ、Kafka）的区别：
- 传统队列：只保证消息投递，不保证工作流状态
- Temporal：保证整个工作流的状态一致性和可恢复性
-->
...
<!--
技术要点：
Claude Agent SDK 是 Shannon 的 AI 推理引擎，主要特点：
1. 流式消息处理 - 实时接收 AI 响应，支持长时间运行的任务
2. MCP（Model Context Protocol）集成 - 通过 MCP 服务器扩展 AI 能力
   - Playwright MCP：浏览器自动化（导航、点击、填表、执行 JS）
   - Shannon Helper MCP：自定义工具（TOTP 生成、配置访问）
3. 工具调用追踪 - 记录所有工具调用和结果，用于审计和调试
4. 成本追踪 - 实时计算 token 使用量和成本
5. 最大自主性配置 - maxTurns=10,000、permissionMode=bypassPermissions

MCP 架构的优势：
- 标准化的工具接口 - 不同 MCP 服务器可以无缝集成
- 隔离性 - 每个 agent 使用独立的 Playwright 实例，避免状态污染
- 可扩展性 - 轻松添加新的工具和能力
-->
...
<!--
技术要点：
Git 检查点机制确保每次 agent 执行都从干净的状态开始：
1. 检查点创建 - 使用 git stash 保存当前工作区状态
2. 失败回滚 - 使用 git stash pop 恢复到检查点状态
3. 成功提交 - 提交更改并删除检查点

优势：
- 原子性 - 每个 agent 的更改要么全部成功，要么全部回滚
- 可追溯 - 每个成功的 agent 都有对应的 git commit
- 隔离性 - 失败的尝试不会污染工作区

与传统备份的区别：
- 传统备份：复制整个目录（慢、占空间）
- Git stash：只保存增量更改（快、高效）
-->
...
<!--
技术要点：
Shannon 的审计系统设计用于生产环境，具有以下关键特性：
1. 崩溃安全 - 即使在 kill -9 的情况下也不会丢失日志
   - 追加式日志：每次写入立即刷新到磁盘
   - 原子操作：session.json 使用临时文件+原子重命名
2. 并发安全 - SessionMutex 确保多个并行 agent 不会产生竞态条件
3. 完整性 - 记录所有关键事件（工具调用、响应、成本、时长）
4. 可复现性 - 保存完整的提示模板，支持精确重现

数据流：
事件发生 → 格式化为 JSON → 追加到日志文件 → 立即刷新
                                    ↓
                            定期更新 session.json（原子操作）

审计日志的用途：
- 成本追踪 - 按 agent、按阶段统计 API 成本
- 性能分析 - 识别耗时最长的 agent 和阶段
- 问题诊断 - 完整的执行轨迹，快速定位问题
- 合规性 - 满足审计要求，证明测试过程
-->
...
<!--
技术要点：
Shannon 的性能优化策略实现了 5 倍的速度提升：
1. 流水线并行化 - 漏洞分析和利用阶段使用流水线并行
   - 传统方式：vuln1→vuln2→vuln3→vuln4→vuln5 → exploit1→exploit2→...
   - Shannon 方式：5 个独立流水线（vuln→exploit）同时运行
   - 关键：每个 exploit 在其对应的 vuln 完成后立即开始，不等待其他 vuln

2. 智能跳过 - 如果漏洞队列为空，跳过对应的 exploit agent
   - 节省不必要的 API 调用
   - 减少总执行时间

3. 资源管理
   - Temporal 心跳机制：防止长时间运行的 agent 被误判为死亡
   - Docker 资源限制：2GB 共享内存用于 Chromium
   - 并发限制：最多 5 个 agent 并行（平衡速度和资源）

性能数据：
- 传统顺序执行：~2.5 小时
- Shannon 流水线并行：~1 小时（60% 提升）
- 成本：相同（$50），但时间减少 60%
-->
